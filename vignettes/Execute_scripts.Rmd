...
The `input` argument can also point to multiple files in a directory using regular expression. In this case, all scripts in the directory will be executed in parallel (and the number of workers can be specified through the `n_workers` argument. Note that the default number of workers is set to one) and a log will be generated for each script. A summary log file will also be provided as a fast way to get an overview of the execution. The location of the summary file can be controlled with the `summary_file` argument.

```{r, eval = FALSE}
# Execution of all R files in a specific directory
run(input = "path/to/directory/*.r|R", 
    n_workers = 4, 
    summary_file = "path/to/summary")


# Execution of all R files starting with "mk200" in a specific directory
run(input = "path/to/directory/mk200*.r|R", 
    n_workers = 8, 
    summary_file = "path/to/summary")
```

More information about regular expression can be found [here](https://stat.ethz.ch/R-manual/R-devel/library/base/html/glob.html).

It is also possible to provide a vector of paths (either single files or regular expression) that should be executed. Note that whenever the `input` argument in is supplied with a vector (e.g. `c("path/to/script1.R", "path/to/script2.R")`) it assumes that these can be executed independently and in parallel. If the elements needs to be executed sequentially this can be achieved by using a `list()` instead (see below).
...
run(input = list("path/to/script1.R", 
                "path/to/script2.R"),
    n_workers = 2)

# In the below example, script1.R and script2.R will be executed in parallel, 
# and all R files in the directory will subsequently be executed in parallel
run(input = list(c("path/to/script1.R", "path/to/script2.R"), 
                 "path/to/directory/*.R"),
    n_workers = 2)
...
The list can also be supplied with names list elements. This can be useful during execution as some of these 'name' will be printed to the console. 

E.g.
```{r, eval = FALSE}
run(input = list(list(name = "Step 1", 
...
The execution order can also be pre-specified in a configuration file (config file for short). The config file could have the following structure. 
