---
title: "`r params$title`"
date: "`r Sys.time()`"
date-format: "YYYY-MM-DDTHH:mm:ss zzz"
params:
  title: 'dev'
  script_md: 'dev'
  p_wd: ""
  strace: FALSE
  strace_path: ''
  strace_discards: NULL
  strace_keep: NULL
  objects_path: ''
  check_approvedpkgs: NULL
  renv: TRUE
format:
  html:
    embed-resources: true
    toc: true
    toc-depth: 2
    toc-expand: true
    smooth-scroll: true
---

# Summary

```{r, echo=FALSE, eval=TRUE}
status <- params$script_md |> 
  whirl:::get_status()

status_txt <- status[c("error", "warning")]
status_txt <- status_txt[lapply(status_txt, length) > 0] |> 
  lapply(\(x) paste("*", x, collapse = "\n")) |> 
  unlist()

whirl:::quarto_callout(
  text = status_txt, 
  title = status$status, 
  type = switch(status$status,
                "error" = "important",
                "warning" = "warning",
                "success" = "tip"
                ), 
  collapse = status$status != "success"
)
```

```{r, echo=FALSE}
if (is.character(params$renv) && params$renv == "yes" | is.logical(params$renv) && params$renv){
  whirl:::renv_status()
}
```

```{r, echo=FALSE}
use_strace <- (is.character(params$strace) && params$strace == "yes" | is.logical(params$strace) && params$strace)

if (use_strace){
  strace_info <- whirl:::read_strace_info(
    path = params$strace_path, 
    p_wd = params$p_wd, 
    strace_discards = params$strace_discards,
    strace_keep = params$strace_keep
  )
}

```

```{r, echo=FALSE, results='asis', eval=use_strace}
cat("## Input", "\n")
```

```{r, echo=FALSE, eval=use_strace}
strace_info$read
```

```{r, echo=FALSE, results='asis', eval=use_strace}
cat("## Output", "\n")
```

```{r, echo=FALSE, eval=use_strace}
strace_info$write
```

# Script

```{r, child = params$script_md}
```

# Session info

```{r, include=FALSE}
knitr::opts_chunk$set(
  error = FALSE,
  warning = FALSE,
  message = FALSE,
  echo = FALSE
)

info <- whirl:::session_info(params$check_approvedpkgs)

```

```{r, results="asis"}
cat('\n## Platform \n')
info$platform
cat("\n")
```


```{r, results="asis", fig.height = 0.5}
cat('\n## Packages\n')

if (!is.null(params$check_approvedpkgs)) {
 # whirl::create_approval_plot(info$packages )
  library("ggplot2")
  library("dplyr")
  info$packages |>
    as.data.frame() |>
    count(Approved) |>
    mutate(pct = prop.table(n),
           status = "Approved",
           lbl = paste0(Approved, ": ", n, "/", sum(n), " (", scales::percent(pct), ")")) |>
    ggplot(aes(x = pct, y = status, fill = Approved, label = lbl)) +
    geom_bar(position = "fill", stat = "identity") +
    geom_text(position = position_stack(vjust = 0.5, reverse = FALSE)) +
    theme_void() +
    theme(legend.position = "none", 
          plot.title = element_text(hjust = 0.5)) +
    scale_fill_manual(values = c("Yes" = "green", "No" = "orange")) + 
    labs(title = "Approved")
}

cat("\n")

info$packages
cat("\n")
```

```{r, results="asis"}
cat('\n## Environment variables \n')
info$environment
cat("\n")
```

```{r, results="asis"}
cat('\n## Option settings\n')
info$options
cat("\n")
```

```{r collect-objects}
whirl_objects <- vector("list")

if (exists("strace_info")) {
  whirl_objects$strace_info <- strace_info
}
if (exists("info")) {
  whirl_objects$environment_options <- info
}
saveRDS(whirl_objects, file = params$objects_path)
```
