...
fetch <- result$session$R |>
  dplyr::filter(directly_used == TRUE) |>
  dplyr::select(dplyr::all_of(c("package", "version", "attached", "source", "approved")))

if (all(is.na(fetch$approved))) {
  fetch[c("package", "version", "attached", "source")]
} else {
  fetch[c("package", "version", "attached", "source", "approved")] |>
...
result$session$R |>
  dplyr::filter(directly_used != TRUE) |>
  dplyr::select(dplyr::all_of(c("package", "version", "attached", "source")))
```
:::
