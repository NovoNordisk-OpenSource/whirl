...
```{r debug, warning=NA}
Sys.setenv("R_LIBS_USER" = Sys.getenv('R_LIBS'))
# .libPaths(c(
#   "/__w/6/library/4.4.1/2024-08-06"
# ))

cli::cli_abort(
  c(
    "Debug information DUM ---------------",
    "options callr: {callr::r_session_options()}",
    "getwd(): {getwd()}",
    ...
    "",
    sprintf("Initial wd (from .First): %s", Sys.getenv("PWD")),
    sprintf("R_ENVIRON_USER: %s", Sys.getenv("R_ENVIRON_USER")),
    sprintf("R_ENVIRON: %s", Sys.getenv("R_ENVIRON")),
    "",
    sprintf(
      "files in getwd(): %s",
      paste(list.files(getwd(), all.files = TRUE), collapse = ", ")
    ),
    "",
    sprintf(".Renviron exists: %s", file.exists(".Renviron")),
    sprintf(".Renviron readable: %s", file.access(".Renviron", 4) == 0),
    "",
    "Debug information DUM end ---------------"
  )
)
```
...
#| error: false
saveRDS(
  object = system('python -m pip list -v', intern = TRUE),
...
#| error: false
info <- list(
  # Due to https://github.com/r-lib/sessioninfo/issues/96
...
if (!"quarto" %in% names(info$platform)) {
  # Due to older versions of sessioninfo
